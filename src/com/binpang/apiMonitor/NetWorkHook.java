		...
		hookhelper.hookMethod(executeRequest, new AbstractBahaviorHookCallBack() {
			...
			public void descParam(HookParam param) {
				...
				if (request instanceof HttpGet) {
					...
					Logger.logI("HTTP Method : " + httpGet.getMethod());
					Logger.logI("HTTP URL : " + httpGet.getURI().toString());
					...
					Logger.logI("HTTP Method : " + httpPost.getMethod());
					Logger.logI("HTTP URL : " + httpPost.getURI().toString());
					Header[] headers = request.getAllHeaders();
					if (headers != null) {
						for (int i = 0; i < headers.length; i++) {
